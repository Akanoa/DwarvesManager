...
import enibdevlab.dwarves.models.levels.Level1;
import enibdevlab.dwarves.models.levels.Tuto1;
import enibdevlab.dwarves.models.levels.Tuto3;
import enibdevlab.dwarves.models.levels.Tuto2;
import enibdevlab.dwarves.models.levels.Tuto4;
import enibdevlab.dwarves.models.levels.Tuto5;
...
public class Loader {
	 ...
	 * Liste des niveaux
	 */
	public static ArrayList<Class<? extends DWLevel>> levels;
	...
	public static void init(){
		...
		mainMenuGuiSmall = new Tileset("data/sprites/mainMenuGuiSmall.png", 256, 64);
		if(DwarvesManager.getWidth()>800 && DwarvesManager.getHeight() > 600){
			mainMenuGui = new Tileset("data/sprites/mainMenuGui.png", 512, 128);
		}
		else{
		...
		parameters = new Class[]{MapArea.class};
		...
		levels = new ArrayList<Class<? extends DWLevel>>();
		levels.add(Tuto1.class);
		levels.add(Tuto2.class);
		levels.add(Tuto3.class);
		levels.add(Tuto4.class);
		levels.add(Tuto5.class);
	 ...
	 */
	public static Room instantiateRoomToPlace(String roomName, MapArea area, boolean usePackageName){
		...
		for(Constructor<?> constructor:rooms){
			if(constructor.getName().substring(roomsPackage.length()).equals(roomName)){
				try {
					return (Room)(constructor.newInstance(area));
				} catch (InstantiationException e) {
					e.printStackTrace();
					continue;
				} catch (IllegalAccessException e) {
