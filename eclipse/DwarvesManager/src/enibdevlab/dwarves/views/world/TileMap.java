...
public class TileMap extends Actor implements IPersistent{
	...
	/**
	 * INTERDIT D'UTILISATION (utile que pour un hack)
	 */
	public TileMap(){}
	...
	public TileMap(Group parent, Level level){
		...
		this.setLevel(level);
		this.level.setTilemap(this);
		
		XmlReader xmlReader = new XmlReader();
		try {
			Element map = xmlReader.parse(Gdx.files.internal("data/maps/" + level.getDwScript().getMapFileName()));
	 ...
	 * @param sy Posiition Y souris
	 */
	public Vector2 getPixelCoordinate(int sx, int sy){
		
		// Calcul des coordonnées à l'écran par rapport à la résolution de base 
		float fsx = (float) (sx*((float)(DwarvesManager.getWidth())/(float)(Gdx.graphics.getWidth()))); 
		float fsy = (float) (sy*((float)(DwarvesManager.getHeight())/(float)(Gdx.graphics.getHeight()))); 
		fsy = DwarvesManager.getHeight()-sy; // car les coords sont inversés en y pour la souris 
	 	// A partir de là on a plus à se préoccuper du redimensionnement, on a les coordonnées de la fenêtre 	                         
	 	// On scrolle les coordonnées pour correspondre à celle de la map 
